/*
    ...
    its citizens with a Walk Generating App on their phones -- everytime you press the button it sends 
...
function repeatChar(walk, char) {
  return (walk.join("").match(new RegExp(`${char}`, "g")) || []).length;
}

function isValidWalk(walk) {
  const n = repeatChar(walk, "n");
  const s = repeatChar(walk, "s");
  const w = repeatChar(walk, "w");
  const e = repeatChar(walk, "e");
